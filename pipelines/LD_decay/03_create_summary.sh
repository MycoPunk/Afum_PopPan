#!/usr/bin/bash
#SBATCH --mem=85G -p batch --nodes 1 --ntasks 2 --out logs/summary.log
...
#All
for N in {1..20}; do \
cat LD_out/n_12_samp${N}_LD_out.ld | sed 1,1d | awk -F " " 'function abs(v) {return v < 0 ? -v : v}BEGIN{OFS="\t"}{print abs($5-$2),$7}' | sort -k1,1n > LD_out/n_12_samp${N}_LD_out.ld.summary
done
