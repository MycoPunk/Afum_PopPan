...
#get genome statitics 

#run other genome stats quick
genome_stats<-read.delim("genome_stats.csv", header = TRUE, sep = ",", fill = TRUE, strip.white = TRUE)
names(genome_stats)
...
n_singletons_by_strain<-read.delim("n_singletons_by_strain.txt", header = TRUE, sep = "\t", fill = TRUE, strip.white = TRUE)
n_accessory_by_strain<-read.delim("n_accessory_by_strain.txt", header = TRUE, sep = "\t", fill = TRUE, strip.white = TRUE)
...
genome_stats$strain.name<- paste("Afum", genome_stats$strain.name, sep ="_")
genome_stats<- data.frame(sapply(genome_stats, gsub, pattern = "Afum_A_fum_", replacement = "Afum_"))
#get stragglers 
...
genome_stats<- data.frame(sapply(genome_stats, gsub, pattern = "Afum_Afu_343_P/11", replacement = "Afum_Afu_343_P_11"))
genome_stats<- data.frame(sapply(genome_stats, gsub, pattern = "Afum_117535A_1.1", replacement = "Afum_117535A_1_1"))
genome_stats<- data.frame(sapply(genome_stats, gsub, pattern = "Afum_B7594CDC_27", replacement = "Afum_B7594_CDC_27"))
genome_stats<- data.frame(sapply(genome_stats, gsub, pattern = "Afum_AFIS1435CDC_6", replacement = "Afum_AFIS1435_CDC_6"))
genome_stats<- data.frame(sapply(genome_stats, gsub, pattern = "Afum_B5258CDC_22", replacement = "Afum_B5258_CDC_22"))
genome_stats<- data.frame(sapply(genome_stats, gsub, pattern = "Afum_B6894CDC_5", replacement = "Afum_B6894_CDC_5"))
genome_stats<- data.frame(sapply(genome_stats, gsub, pattern = "Afum_B5960CDC_29", replacement = "Afum_B5960_CDC_29"))
