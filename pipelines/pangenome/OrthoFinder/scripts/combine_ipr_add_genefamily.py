...
parser.add_argument('--ipr', help='Interpro TSV file',required=False,default='Af.pan.genome.iprout.tsv')
parser.add_argument('--peps',help="Protein sequence file with extra headers to include",required=False,default="Orthologs.Longest.fa")
...
pepid = {}
capture = re.compile(r'^>(\S+)\s+(\S+)')
with open(args.peps,"r") as inpep:
    for line in inpep:
        if line.startswith(">"):
            m = capture.match(line)
            if m:
                pepid[m.group(1)] = [m.group(2)]
